...
For Linux drag-and-drop, the WebView first uses the supplied `File`, including legacy WebKit objects without `Blob.arrayBuffer()`. If the client supplies only `text/uri-list`, the application also tries to read the explicitly dropped local `file:` URI. When that access is supported, choosing the topic and caption uses the original drop, so the file does not need to be selected again. If Telegram’s sandbox does not allow the page to read even that URI, the upload dialog retains the system picker as the unavoidable fallback. Windows keeps its direct drop path.

AI JSON export does not call Telegram Desktop for Linux’s advertised but broken save picker, avoiding its native “failed to get file” error. It copies the JSON directly to the clipboard and shows the suggested filename with instructions to save the pasted text manually. Other browsers continue to use the system save dialog or a normal download. Without a backend hosting a temporary public HTTPS file, a static Mini App cannot pass a local `blob:` or `data:` URL to Telegram’s system downloader.
