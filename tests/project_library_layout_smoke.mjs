...
assert.match(css, /grid-template-columns:\s*min\(var\(--project-library-left-width\),\s*30vw\)\s+6px\s+minmax\(0,\s*1fr\)\s+6px\s+min\(var\(--project-library-right-width\),\s*32vw\)/);
assert.match(layout, /this\.db\.put\("settings", SETTINGS_KEY, serializeRatios\(this\.ratios\)\)/, "panel ratios must persist per tab");
assert.match(layout, /project:\s*Object\.freeze\(\["projectLibraryLeft", "projectLibraryRight"\]\)/);
...
assert.match(css, /\.project-library-item\.active-editor\s*\{[\s\S]*?border-color:\s*var\(--theme-success-border, #59ae78\)/);
assert.match(css, /\.project-library-detail \.project-post-card\.active\s*\{[\s\S]*?border-color:\s*var\(--theme-success-border, #59ae78\)/);
